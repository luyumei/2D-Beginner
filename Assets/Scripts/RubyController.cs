...

{
    ...
    // if you make currentHealth public, you could change it to 10 in another script, even if the maxHealth is just 5.
    ...
    {
        rigidbody2D =
            GetComponent<Rigidbody2D>(); // give you the Rigidbody2D that is attached to the same GameObject that your script is attached to, which is your character

        currentHealth = maxHealth; // character starts with max Health
    ...
    private void FixedUpdate()
    {
        Vector2
            position = rigidbody2D
                .position; // use the Rigidbody itself instead of GameObject Transform (to avoid Ruby’s jittering)
        ...
        position.y =
            position.y +
            speed * vertical *
            Time.deltaTime; // The y value stored inside a Vector2 corresponds to the vertical position
        rigidbody2D.MovePosition(
            position); // MovePosition (Vector2 newPosition)
    ...
    {
        currentHealth =
            Mathf.Clamp(currentHealth + amount, 0,
                maxHealth); // Clamping ensures that the first parameter never goes lower than the second parameter
